...
{
    [TestFixture]
    public class OnViewMakeAdmin_Should
    {
        [Test]
        public void Call_UserServiceMakeAdminMethodOnceWithCorrectParams()
        {
            ...
            userService.Setup(s => s.MakeAdmin(args.Id))
                       .Returns(IdentityResult.Success);

            var sut = new ManageUsersPresenter(userService.Object, view.Object);
            view.Raise(v => v.UserMakeAdmin += null, view.Object, args);

            userService.Verify(s => s.MakeAdmin(args.Id), Times.Once);
        }
